											 ...
											 TupleDesc chunk_desc)
{
	...
	foreach(lc, update_tles)
	{
		...
		if (strcmp(NameStr(attribute->attname), t->resname) != 0)
			elog(ERROR, "TLE order in onConflictUpdate does not match chunk attribute order");
...
adjust_projections(ChunkInsertState *cis, ChunkDispatch *dispatch, Oid rowtype)
{
	...
	if (rri->ri_projectReturning != NULL)
	{
		rri->ri_projectReturning =
			get_adjusted_projection_info_returning(
												   rri->ri_projectReturning, list_nth(dispatch->returning_lists, dispatch->returning_index),
												   variable_attnos_map, variable_attnos_map_size,
												   dispatch->hypertable_result_rel_info->ri_RangeTableIndex, rowtype,
												   chunk_desc);
	}
	...
	if (rri->ri_onConflictSetProj != NULL)
	{
		rri->ri_onConflictSetProj =
			get_adjusted_projection_info_onconflicupdate(
														 rri->ri_onConflictSetProj, dispatch->on_conflict_set, variable_attnos_map,
		...
		if (rri->ri_onConflictSetWhere != NULL)
		{
			...
			rri->ri_onConflictSetWhere =
				...
				get_adjusted_onconflictupdate_where((ExprState *)
													rri->ri_onConflictSetWhere, dispatch->on_conflict_where, variable_attnos_map,
													variable_attnos_map_size,
